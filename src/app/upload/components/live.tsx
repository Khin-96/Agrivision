  ...
  return (
    <group>
      <points ref={pointsRef}>
        <bufferGeometry>
          {/* FIXED: use args prop so TypeScript/types for R3F are satisfied */}
          <bufferAttribute attach="attributes-position" args={[positions, 3]} />
        </bufferGeometry>
        <pointsMaterial
          size={0.02 + amplitude * 0.02}
          sizeAttenuation
          color="#3ee07a"
          transparent
          opacity={0.95}
  ...
  useEffect(() => {
    ...
    const sendFrame = () => {
      ...
      if (!ws || ws.readyState !== WebSocket.OPEN || !video)
        return setTimeout(sendFrame, 1000);

      canvas.width = 960;
      canvas.height =
        Math.round((video.videoHeight / video.videoWidth) * 960) || 540;
      ctx?.drawImage(video, 0, 0, canvas.width, canvas.height);
      ws.send(
        JSON.stringify({ type: "frame", data: canvas.toDataURL("image/jpeg", 0.55) })
      );
  ...
  return (
    <div className="fixed inset-0 flex items-stretch justify-center bg-black">
      ...
      <button
        ...
        className="absolute top-6 right-6 z-50 text-white text-2xl bg-white/10 hover:bg-white/20 rounded-full w-10 h-10 flex items-center justify-center transition"
      >
        ✕
      </button>
      ...
      <div className="absolute left-8 bottom-8 z-40 max-w-lg w-full flex flex-col-reverse gap-3">
        <AnimatePresence initial={false}>
          {messages.map((m) => (
            ...
            >
              <div
                className="p-4 rounded-2xl backdrop-blur-md bg-white/10 border border-white/10 text-white shadow-lg"
                style={{ boxShadow: "0 8px 25px rgba(2,6,23,0.45)" }}
              >
